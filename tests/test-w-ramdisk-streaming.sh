...
(
    cd "$source_directory"
    "$timeout_command" 120 "$executable" a "$archive_path" payload.bin -m1 -noeta -nocolor
)
...
echo "PASS: w -ramdisk restored a ${file_size_mib} MiB file with a ${ram_budget_mib} MiB RAM chunk budget."
